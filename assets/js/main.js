...
(function () {
  ...
  (function openStatus() {
    ...
    function render() {
      ...
      if (today && mins >= today[0] * 60 && mins < today[1] * 60) {
        ...
        text = 'Откроемся сегодня в' + NB + time(today[0]);
      } else {
        var nx = nextDay(day);
        text = nx.shift === 1
          ? 'Откроемся завтра в' + NB + time(nx.hour)
          : 'Откроемся в' + NB + DAYS[nx.day] + ' в' + NB + time(nx.hour);
      }
  ...
  /* ─────────── Формы заявки ─────────── */
  /* Куда отправлять заявку: адрес посредника относительно страницы (…/v2/api/lead) */
  ...
  document.querySelectorAll('.js-lead-form').forEach(function (form) {
    ...
    form.addEventListener('submit', function (e) {
      ...
      /* ─────────────────────────────────────────────────────────
         Заявка уходит в мессенджер MAX через посредника на сервере (LEAD_ENDPOINT).
         Токен бота хранится только там — в коде страницы его нет и быть не должно.
         Локально посредник — server.ps1, на хостинге нужен такой же адрес api/lead.
         Ответ ждём не дольше 15 секунд, иначе предлагаем позвонить.
         ───────────────────────────────────────────────────────── */
      var timeout = new Promise(function (_, reject) { setTimeout(reject, 15000); });
      var request = fetch(LEAD_ENDPOINT, {
        ...
        return r.json().catch(function () { return {}; }).then(function (json) {
          if (!r.ok || !json.ok) throw new Error('lead ' + r.status);
        });
      });
      Promise.race([request, timeout]).then(done, fail);
      ...
      function fail() {
        ...
        status.textContent = 'Не получилось отправить заявку. Позвоните нам: +7' + NB + '(904)' + NB + '272-31-47';
      }
    });
  });
